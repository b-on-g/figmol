...
(function ($) {
     ...
     */
    const figmol_store_test_props = (store) => {
        const values = {};
        store.prop_read = (id, key) => values[id + ':' + key] ?? '';
        store.prop_write = (id, key, val) => { values[id + ':' + key] = val; };
        return values;
    };
    const figmol_store_test_nums = (store) => {
        const values = {};
        store.num_read = (id, field) => values[id + ':' + field] ?? 0;
        store.num_write = (id, field, val) => { values[id + ':' + field] = val; };
        return values;
    };
    /** A frame with three children, none of which is anywhere near the Baza. */
    const figmol_store_test_kids = (store, kids) => {
        store.parent = () => 'root';
        store.kids = (id) => id === 'root' ? kids : [];
        store.kid_cut = (host, id) => {
            kids.splice(0, kids.length, ...kids.filter(kid => kid !== id));
        };
        store.kid_put = (host, id, at) => {
...
(function ($) {
    ...
    $mol_test({
        'a name is one lowercase word'() {
            const error = (name) => $bog_figmol_deploy_github.name_error(name);
        ...
        'a tree carries every file inline, in a stable order'() {
            ...
            $mol_assert_like(body.tree.map(item => item.path), ['.github/workflows/deploy.yml', 'index.html', 'mysite.view.tree']);
        ...
        'a tree without a base replaces the whole repository'() {
            ...
            $mol_assert_equal(clean.base_tree, undefined);
            const layered = $bog_figmol_deploy_github.tree_body(figmol_deploy_test_files, 'abc123');
            $mol_assert_equal(layered.base_tree, 'abc123');
        },
        'a first commit has no parent, a next one has'() {
            $mol_assert_like($bog_figmol_deploy_github.commit_body('Published', 'tree1', []), { message: 'Published', tree: 'tree1', parents: [] });
            $mol_assert_like($bog_figmol_deploy_github.commit_body('Published', 'tree2', ['head1']), { message: 'Published', tree: 'tree2', parents: ['head1'] });
        },
        ...
        'pages are served from the root of the branch the workflow writes'() {
            $mol_assert_like($bog_figmol_deploy_github.pages_body('gh-pages'), { build_type: 'legacy', source: { branch: 'gh-pages', path: '/' } });
        },
        'a reference names a branch in full'() {
            $mol_assert_like($bog_figmol_deploy_github.ref_body('main', 'sha1'), { ref: 'refs/heads/main', sha: 'sha1' });
        },
        'a run is read as waiting, working, done or failed'() {
            const state = (run) => $bog_figmol_deploy_github.run_state(run);
        ...
        'a site lives in a folder, unless the repository is the account page'() {
            $mol_assert_equal($bog_figmol_deploy_github.site_uri('Alice', 'mysite'), 'https://alice.github.io/mysite/');
        ...
        'a refusal keeps the wording GitHub used'() {
            ...
            $mol_assert_equal($bog_figmol_deploy_github.fail(401, JSON.stringify({ message: 'Bad credentials' })), 'GitHub 401: Bad credentials');
...
(function ($) {
    ...
    const figmol_gen_test_sign = (name) => '$' + name;
    const figmol_gen_test_site = {
        title: 'My Site',
        theme: {
            back: '#ffffff',
            text: '#111111',
    ...
    $mol_test({
        'every file of a repository is generated'() {
            ...
            $mol_assert_like(Object.keys(files).sort(), [
                '.gitattributes',
...
(function ($) {
    $mol_test({
        ...
        'an avatar with no picture draws a placeholder instead'() {
            const blank = new $bog_figmol_app_canvas_shape;
            blank.kind = () => 'bui_avatar';
            blank.uri = () => '';
            $mol_assert_equal(blank.content()[0], blank.Avatar_stub());
            const filled = new $bog_figmol_app_canvas_shape;
        ...
        'shape of a frame draws its kids'() {
            const kid = new $bog_figmol_app_canvas_shape;
            const frame = new $bog_figmol_app_canvas_shape;
            frame.kind = () => 'frame';
            frame.kids = () => [kid];
        ...
        'shape of a card hands its children over instead of drawing them'() {
            const kid = new $bog_figmol_app_canvas_shape;
            const card = new $bog_figmol_app_canvas_shape;
            card.kind = () => 'bui_card';
            card.kids = () => [kid];
        ...
        'a layer row quotes the caption of its element'() {
            const layers = new $bog_figmol_app_layers;
            const store = layers.store();
            store.kind = (id) => id === 'a' ? 'bui_card' : 'text';
            store.text = (id) => id === 'a' ? '' : 'A caption long enough to be cut short';
            store.depth = (id) => id === 'a' ? 0 : 1;
        ...
        'canvas keeps the point under the cursor while zooming'() {
            ...
            $mol_assert_equal(Math.round((300 - canvas.pan_x()) / canvas.zoom()), Math.round(before));
        },
        /**
        ...
        'a block from the palette lands in the middle of the viewport'() {
            const blocks = new $bog_figmol_app_blocks;
            blocks.spot = () => [500, 300];
